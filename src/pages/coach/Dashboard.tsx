...
export default function CoachDashboard() {
  ...
  const [coachView, setCoachView] = useState<"seances" | "gestion">("seances");
  ...
  if (!activeAthleteId) {
    return (
      <div style={{ minHeight: "100vh", background: C.bg, padding: "0 16px 40px" }}>
        <div style={{ maxWidth: 480, margin: "0 auto", paddingTop: 28 }}>
          ...
          {/* Tab toggle */}
          <div style={{ display: "flex", gap: 6, marginBottom: 20, background: C.s1, borderRadius: 10, padding: 4, border: "1px solid " + C.brd }}>
            {(["seances", "gestion"] as const).map(tab => (
              <button
                key={tab}
                onClick={() => setCoachView(tab)}
                style={{
                  flex: 1, padding: "8px 0", borderRadius: 8, cursor: "pointer",
                  fontFamily: "inherit", fontSize: 13, fontWeight: 600,
                  border: "none",
                  background: coachView === tab ? C.coach : "transparent",
                  color: coachView === tab ? "#fff" : C.tx3,
                  transition: "all 0.15s",
                }}
              >
                {tab === "seances" ? "Séances" : "Gestion"}
              </button>
            ))}
          </div>
                ...
                {athletes.map(a => (
                  <div key={a.id} style={{ marginBottom: 8 }}>
                    <button onClick={() => setActiveAthleteId(a.id)}
                      style={{ width: "100%", display: "flex", alignItems: "center", gap: 12, padding: "12px", borderRadius: 10, border: "1px solid " + C.brdL, background: C.s2, cursor: "pointer", fontFamily: "inherit", textAlign: "left" }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 11, color: C.tx3 }}>
                          ...
                          {!a.age && !a.height_cm && "Profil incomplet"}
                        </div>
                      </div>
              ...
              {inviteLink && (
                <div style={{ marginTop: 8, padding: "10px 12px", borderRadius: 8, background: C.s2, border: "1px solid " + C.brdL, display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <button onClick={async () => { await navigator.clipboard.writeText(inviteLink); setCopyMsg("Lien copié !"); setTimeout(() => setCopyMsg(""), 2500); }}
                    style={{ flexShrink: 0, padding: "6px 12px", borderRadius: 8, border: "1px solid " + (copyMsg ? C.g : C.brdL), background: copyMsg ? "rgba(34,201,147,0.12)" : C.s1, color: copyMsg ? C.g : C.tx2, fontSize: 11, fontWeight: 600, cursor: copyMsg ? "default" : "pointer", fontFamily: "inherit" }}>
            ...
            <>
              {athletes.length === 0 && !isCoachAthlete ? (
                <div style={{ textAlign: "center", padding: "48px 0", color: C.tx3, fontSize: 13 }}>
                  Aucun athlète pour l'instant.<br />
                  <span style={{ fontSize: 12 }}>Passe en vue Séances pour partager ton code.</span>
                </div>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  {athletes.map(a => {
                    ...
                    const hasProfile = !!(a.first_name || a.age || a.height_cm);
                    return (
                      <div key={a.id} style={{ background: C.s1, borderRadius: 14, border: "1px solid " + C.brd, overflow: "hidden" }}>
                        {/* Card header */}
                        ...
                        {hasProfile ? (
                          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 1, background: C.brd, borderTop: "1px solid " + C.brd }}>
                            {[
                              ...
                              <div key={stat.label} style={{ background: C.s2, padding: "10px 12px", textAlign: "center" }}>
                          ...
                          <div style={{ padding: "10px 16px", background: C.s2, borderTop: "1px solid " + C.brd }}>
                        ...
                        <div style={{ display: "flex", gap: 8, padding: "12px 16px", borderTop: "1px solid " + C.brd }}>
                          <button
                            ...
                            style={{ flex: 1, padding: "8px 0", borderRadius: 8, border: "1px solid " + C.coach + "50", background: C.coachS, color: C.coach, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}
                          >
                            ✎ Modifier profil
                          </button>
                          <button
                            onClick={() => setActiveAthleteId(a.id)}
                            style={{ flex: 1, padding: "8px 0", borderRadius: 8, border: "1px solid " + C.brdL, background: C.s2, color: C.tx2, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}
                          >
                            Voir séances ›
                          </button>
                          {confirmRemove === a.id ? (
                            <div style={{ display: "flex", gap: 6 }}>
                              <button onClick={() => setConfirmRemove(null)} style={{ padding: "8px 10px", borderRadius: 8, border: "1px solid " + C.brdL, background: "transparent", color: C.tx3, fontSize: 11, cursor: "pointer", fontFamily: "inherit" }}>✕</button>
                              <button onClick={() => handleRemoveAthlete(a.id)} disabled={removing} style={{ padding: "8px 10px", borderRadius: 8, border: "none", background: C.r, color: "#fff", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>OK</button>
                            </div>
                          ) : (
                            <button
                              onClick={() => setConfirmRemove(a.id)}
                              style={{ width: 36, borderRadius: 8, border: "1px solid " + C.r + "40", background: C.r + "12", color: C.r, fontSize: 16, cursor: "pointer", fontFamily: "inherit", flexShrink: 0 }}
                            >
                              ×
                            </button>
                          )}
