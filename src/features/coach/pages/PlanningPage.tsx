...
// Vue Bloc (réutilise ProgPage inline pour éviter la duplication)
...
export default function PlanningPage() {
  ...
  const view = (searchParams.get("view") as PlanView) ?? "block";
  ...
  const {
    ...
    wellnessHistory, sessionLogs, weightLog, nutritionLog, nutritionStrategy,
  ...
  return (
    <div style={{ padding: "16px 24px 60px" }}>
      ...
      {/* ── JOUR ──────────────────────────────────────────────────────────── */}
      {view === "day" && <DayView sessions={sessions} exos={exos} completedSessions={completedSessions} currentWeek={currentWeek} />}
    </div>
  );
}

// ── DayView — vue détail du jour ──────────────────────────────────────────────
            ...
            return (
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                  <div
                    style={{
                      fontSize: 12, fontWeight: 700, color: done ? C.g : C.tx,
                      flex: 1,
                    }}
                  >
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  {exercises.slice(0, 5).map((ex, i) => (
                    <div
                      key={i}
                      style={{
                        fontSize: 12, color: C.tx2, display: "flex",
