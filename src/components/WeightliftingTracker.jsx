  ...
  return(<div>
    ...
    {rows.map((r,i)=>{const isIso=r.type==="iso";const isAmrap=r.type==="amrap";const isDrop=r.type==="drop";const isMini=r.type==="mini";const showPause=(r.type==="round"||r.type==="mini")&&i<rows.length-1&&rows[i+1]?.type===r.type;
      return(<div key={i}><div style={{display:"grid",gridTemplateColumns:isIso?"40px 1fr 28px 28px":"40px 1fr 10px 1fr "+((!isAmrap&&!isMini&&!isDrop)?"44px ":"")+"28px 28px",gap:4,alignItems:"center",marginBottom:4,padding:"6px 8px",borderRadius:8,background:r.done?C.g+"10":r.skipped?C.tx3+"08":isDrop?C.o+"08":isMini?C.ac+"08":C.s2,border:"1px solid "+(r.done?C.g+"30":r.skipped?C.tx3+"20":isDrop?C.o+"20":isMini?C.ac+"20":C.brd),opacity:r.done||r.skipped?0.5:1,transition:"all 0.2s"}}>
        ...
        {!isIso&&<input type="number" value={r.kg||0} onChange={viewOnly?undefined:e=>upd(i,"kg",+e.target.value)} readOnly={viewOnly} style={iS}/>}
...
function LogView({exos,sets,updSets,completedSessions,completeSession,uncompleteSession,goals,currentWeek,allMethods,athleteNotes,setAthleteNotes,sessions,blockConfig,initialSess=null,timerLeft,timerDur,timerActive,timerFinished,onTimerSetDur,onTimerStart,onTimerStop,viewOnly=false}){
  ...
  if(step===0){
    ...
    <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:20}}>{sessions.map(s=>{const done=(completedSessions[wk]||[]).includes(s.id);const sessExos=exos[s.id]||[];const hasExos=sessExos.length>0;const exosDone=sessExos.filter(ex=>{const sk=ex.id+"_"+wk;const rows=sets[sk]||[];return rows.length>0&&rows.every(r=>r.done||r.skipped);}).length;const muscles=[...new Set(sessExos.map(e=>e.target))];
      return(<button key={s.id} onClick={()=>{if(!done&&hasExos&&!viewOnly){setSelectedSess(s);setStep(1);setOpenEx(null);}}} style={{width:"100%",padding:0,borderRadius:14,border:"1.5px solid "+(done?C.g+"40":C.brd),background:C.s1,cursor:done||!hasExos||viewOnly?"default":"pointer",fontFamily:"inherit",textAlign:"center",display:"block",opacity:hasExos?1:0.35,overflow:"hidden",position:"relative",transition:"all 0.2s",boxSizing:"border-box"}}>
...
export default function App({athleteId,defaultMode,canToggleMode=true,userName,athleteProfile,onEditProfile,viewOnly=false}){
  ...
  return(<div style={{background:C.s2,borderRadius:12,padding:"14px 12px"}}>
    ...
    <div style={{fontSize:9,color:reached?C.g:C.tx3,marginTop:5,fontWeight:reached?600:400}}>{deltaLabel}</div>
      ...
      {tab==="alim"&&<NutritionView athleteId={athleteId} bmr={athleteProfile?.base_metabolism||null} nutritionStrategy={nutritionStrategy} onLogSaved={(date,log)=>setNutritionLogState(prev=>({...prev,[date]:log}))} viewOnly={viewOnly}/>}
