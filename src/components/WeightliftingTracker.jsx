...
function NewBlockModal({onStart,onClose,onResume,hasCurrentData,blockHistory=[],onDelete,currentAthleteId}){
  ...
  const[copyAthletes,setCopyAthletes]=useState([]);
  const[copyAthLoading,setCopyAthLoading]=useState(false);
  const[selectedCopyAth,setSelectedCopyAth]=useState(null);
  const[copyBlocks,setCopyBlocks]=useState([]);
  const[copyBlocksLoading,setCopyBlocksLoading]=useState(false);
  ...
  const openCopyPanel=async()=>{setCopyStep('athletes');if(copyAthletes.length>0)return;setCopyAthLoading(true);const{data:prof}=await supabase.from('profiles').select('coach_id,role').eq('id',currentAthleteId).maybeSingle();const coachId=prof?.coach_id||(prof?.role==='coach'||prof?.role==='coach_athlete'?currentAthleteId:null);if(coachId){const{data:aths}=await supabase.from('profiles').select('id,full_name').eq('coach_id',coachId).neq('id',currentAthleteId);setCopyAthletes(aths||[]);}setCopyAthLoading(false);};
  const selectCopyAth=async(ath)=>{setSelectedCopyAth(ath);setCopyStep('blocks');setCopyBlocksLoading(true);const[{data:histData},{data:sessData},{data:exosData},{data:bcData}]=await Promise.all([supabase.from('app_data').select('value').eq('athlete_id',ath.id).eq('key','asp:blockHistory').maybeSingle(),supabase.from('app_data').select('value').eq('athlete_id',ath.id).eq('key','asp:sessions').maybeSingle(),supabase.from('app_data').select('value').eq('athlete_id',ath.id).eq('key','asp:exos').maybeSingle(),supabase.from('app_data').select('value').eq('athlete_id',ath.id).eq('key','asp:blockConfig').maybeSingle()]);const history=histData?.value||[];const hasCurrent=Array.isArray(sessData?.value)&&sessData.value.length>0&&exosData?.value&&Object.values(exosData.value).flat().length>0;const allBlocks=hasCurrent?[{sessions:sessData.value,exos:exosData.value,blockConfig:bcData?.value,completedSessions:{},_isCurrent:true},...history]:history;setCopyBlocks(allBlocks);setCopyBlocksLoading(false);};
  const closeCopyPanel=()=>{setCopyStep(null);setSelectedCopyAth(null);};
  ...
  return(<div style={{position:"fixed",inset:0,zIndex:250,background:C.bg,overflowY:"auto"}}>
    ...
    <div style={{padding:16}}>
      ...
      {step===0&&(<>
        ...
        <button onClick={()=>copyStep?closeCopyPanel():openCopyPanel()} style={{width:"100%",display:"flex",alignItems:"center",gap:12,padding:"14px 16px",borderRadius:12,border:"1px solid "+C.o+"50",background:copyStep?C.oS:"transparent",marginBottom:copyStep?0:10,cursor:"pointer",fontFamily:"inherit",textAlign:"left"}}>
          <div style={{width:36,height:36,borderRadius:10,background:C.o+"20",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>📋</div>
          <div><div style={{fontSize:13,fontWeight:700,color:C.o}}>Copier le bloc d'un autre athlète</div><div style={{fontSize:11,color:C.tx3}}>Partir du programme d'un de tes athlètes</div></div>
        </button>
        {copyStep&&(<div style={{marginBottom:10,borderRadius:10,border:"1px solid "+C.brd,overflow:"hidden"}}>
          {copyStep==='athletes'&&(<>{copyAthLoading&&<div style={{padding:"14px",fontSize:11,color:C.tx3,textAlign:"center"}}>Chargement...</div>}{!copyAthLoading&&copyAthletes.length===0&&<div style={{padding:"14px",fontSize:11,color:C.tx3,textAlign:"center"}}>Aucun autre athlète</div>}{copyAthletes.map(ath=>(<button key={ath.id} onClick={()=>selectCopyAth(ath)} style={{width:"100%",display:"flex",alignItems:"center",gap:10,padding:"11px 14px",background:"transparent",border:"none",borderBottom:"1px solid "+C.brd,cursor:"pointer",fontFamily:"inherit",textAlign:"left"}}><div style={{width:28,height:28,borderRadius:"50%",background:C.o+"20",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:700,color:C.o,flexShrink:0}}>{ath.full_name.charAt(0).toUpperCase()}</div><span style={{flex:1,fontSize:12,fontWeight:600,color:C.tx}}>{ath.full_name}</span><span style={{fontSize:11,color:C.tx3}}>→</span></button>))}</>)}
          {copyStep==='blocks'&&(<><div style={{display:"flex",alignItems:"center",gap:8,padding:"10px 14px",borderBottom:"1px solid "+C.brd,background:C.s2}}><button onClick={()=>{setCopyStep('athletes');setSelectedCopyAth(null);}} style={{background:"none",border:"none",color:C.tx3,cursor:"pointer",fontFamily:"inherit",fontSize:16,padding:"0 2px"}}>‹</button><span style={{fontSize:11,fontWeight:600,color:C.tx2}}>{selectedCopyAth?.full_name}</span></div>{copyBlocksLoading&&<div style={{padding:"14px",fontSize:11,color:C.tx3,textAlign:"center"}}>Chargement...</div>}{!copyBlocksLoading&&copyBlocks.length===0&&<div style={{padding:"14px",fontSize:11,color:C.tx3,textAlign:"center"}}>Aucun programme pour cet athlète</div>}{!copyBlocksLoading&&copyBlocks.map((block,i)=>{const isCur=block._isCurrent;const name=block.blockConfig?.blockName||(isCur?"Bloc en cours":"Bloc "+(copyBlocks.length-i));const date=isCur?"En cours":block.archivedAt?new Date(block.archivedAt).toLocaleDateString("fr-FR",{day:"numeric",month:"short"}):"";const totalDone=isCur?((block.sessions||[]).length+" séances planifiées"):Object.values(block.completedSessions||{}).flat().length+" séances réalisées";return(<button key={block.id||i} onClick={()=>{loadFromHistory(block);closeCopyPanel();}} style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"11px 14px",background:isCur?C.gS:"transparent",border:"none",borderBottom:"1px solid "+C.brd,cursor:"pointer",fontFamily:"inherit",textAlign:"left"}}><div><div style={{fontSize:12,fontWeight:600,color:isCur?C.g:C.tx}}>{name}{isCur&&<span style={{fontSize:9,marginLeft:6,padding:"1px 5px",borderRadius:4,background:C.g+"25",color:C.g,fontWeight:700}}>ACTIF</span>}</div><div style={{fontSize:10,color:C.tx3}}>{date}{date&&" · "}{block.blockConfig?.totalWeeks||6} sem. · {totalDone}</div></div><span style={{fontSize:11,color:C.o,fontWeight:600,flexShrink:0,marginLeft:8}}>Copier →</span></button>);})}</>)}
        </div>)}
      ...
      {step===1&&(<>
        ...
        {currentAthleteId&&(<div style={{marginBottom:16}}>
          {copyStep?(
            <div style={{padding:"10px 14px",borderRadius:10,background:C.oS,border:"1px solid "+C.o+"40",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              <div style={{fontSize:11,color:C.o,fontWeight:600}}>📋 Basé sur le bloc d'un autre athlète</div>
              <button onClick={()=>{closeCopyPanel();if(fromHistory){setFromHistory(false);setRestoredExos(null);setBlockName("");setObjective("");setNewSessions([]);}}} style={{fontSize:10,color:C.r,background:"none",border:"none",cursor:"pointer",fontFamily:"inherit",fontWeight:600}}>Effacer</button>
            </div>
          ):(
            <button onClick={()=>copyStep?closeCopyPanel():openCopyPanel()} style={{width:"100%",display:"flex",alignItems:"center",gap:10,padding:"11px 14px",borderRadius:10,border:"1px solid "+C.o+"40",background:"transparent",cursor:"pointer",fontFamily:"inherit",textAlign:"left"}}>
              <span style={{fontSize:16}}>📋</span>
              <div style={{flex:1}}><div style={{fontSize:12,fontWeight:600,color:C.o}}>Copier depuis un autre athlète</div><div style={{fontSize:10,color:C.tx3}}>Partir du programme d'un de tes athlètes</div></div>
            </button>
          )}
          {copyStep&&!fromHistory&&(<div style={{marginTop:6,borderRadius:10,border:"1px solid "+C.brd,overflow:"hidden"}}>
            {copyStep==='athletes'&&(<>{copyAthLoading&&<div style={{padding:"14px",fontSize:11,color:C.tx3,textAlign:"center"}}>Chargement...</div>}{!copyAthLoading&&copyAthletes.length===0&&<div style={{padding:"14px",fontSize:11,color:C.tx3,textAlign:"center"}}>Aucun autre athlète</div>}{copyAthletes.map(ath=>(<button key={ath.id} onClick={()=>selectCopyAth(ath)} style={{width:"100%",display:"flex",alignItems:"center",gap:10,padding:"11px 14px",background:"transparent",border:"none",borderBottom:"1px solid "+C.brd,cursor:"pointer",fontFamily:"inherit",textAlign:"left"}}><div style={{width:28,height:28,borderRadius:"50%",background:C.o+"20",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:700,color:C.o,flexShrink:0}}>{ath.full_name.charAt(0).toUpperCase()}</div><span style={{flex:1,fontSize:12,fontWeight:600,color:C.tx}}>{ath.full_name}</span><span style={{fontSize:11,color:C.tx3}}>→</span></button>))}</>)}
            {copyStep==='blocks'&&(<><div style={{display:"flex",alignItems:"center",gap:8,padding:"10px 14px",borderBottom:"1px solid "+C.brd,background:C.s2}}><button onClick={()=>{setCopyStep('athletes');setSelectedCopyAth(null);}} style={{background:"none",border:"none",color:C.tx3,cursor:"pointer",fontFamily:"inherit",fontSize:16,padding:"0 2px"}}>‹</button><span style={{fontSize:11,fontWeight:600,color:C.tx2}}>{selectedCopyAth?.full_name}</span></div>{copyBlocksLoading&&<div style={{padding:"14px",fontSize:11,color:C.tx3,textAlign:"center"}}>Chargement...</div>}{!copyBlocksLoading&&copyBlocks.length===0&&<div style={{padding:"14px",fontSize:11,color:C.tx3,textAlign:"center"}}>Aucun programme pour cet athlète</div>}{!copyBlocksLoading&&copyBlocks.map((block,i)=>{const isCur=block._isCurrent;const name=block.blockConfig?.blockName||(isCur?"Bloc en cours":"Bloc "+(copyBlocks.length-i));const date=isCur?"En cours":block.archivedAt?new Date(block.archivedAt).toLocaleDateString("fr-FR",{day:"numeric",month:"short"}):"";const totalDone=isCur?((block.sessions||[]).length+" séances planifiées"):Object.values(block.completedSessions||{}).flat().length+" séances réalisées";return(<button key={block.id||i} onClick={()=>{loadFromHistory(block);closeCopyPanel();}} style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"11px 14px",background:isCur?C.gS:"transparent",border:"none",borderBottom:"1px solid "+C.brd,cursor:"pointer",fontFamily:"inherit",textAlign:"left"}}><div><div style={{fontSize:12,fontWeight:600,color:isCur?C.g:C.tx}}>{name}{isCur&&<span style={{fontSize:9,marginLeft:6,padding:"1px 5px",borderRadius:4,background:C.g+"25",color:C.g,fontWeight:700}}>ACTIF</span>}</div><div style={{fontSize:10,color:C.tx3}}>{date}{date&&" · "}{block.blockConfig?.totalWeeks||6} sem. · {totalDone}</div></div><span style={{fontSize:11,color:C.o,fontWeight:600,flexShrink:0,marginLeft:8}}>Copier →</span></button>);})}</>)}
          </div>)}
        </div>)}
